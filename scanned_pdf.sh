...
convert tmp.png \( "${SIGN_FILE}" -rotate $(seq -1.5 .01 1.5 | shuf | head -n1) \) -gravity Center -geometry 200x200-180+530 -composite tmp.png
composite -dissolve 3 -gravity Center "${BACKGR_FILE}" tmp.png -alpha Set tmp.png
convert tmp.png -linear-stretch 3.5%x10% -blur 0x0.5 -attenuate 0.25 +noise Gaussian  -rotate $(seq -0.3 .01 0.3 | shuf | head -n1) aux_output.pdf
